...
export default class ExpensiMark {
    ...
    modifyTextForUrlLinks(regex, textToCheck, replacement) {
        ...
        while (match !== null) {
            ...
            if (match[12] !== undefined && !_.contains(TLD_REGEX.split('|'), match[12].toUpperCase())) {
                abort = true;
            }
            ...
            if (abort || match[1].includes('<pre>')) {
                ...
                const urlMatch = urlRegex.exec(match[1]);

                let isUrl = false;
                if (urlMatch !== null) {
                    isUrl = true;

                    if (urlMatch[10] !== undefined && !_.contains(TLD_REGEX.split('|'), urlMatch[10].toUpperCase())) {
                        isUrl = false;
                    }
                }
                ...
                const linkText = isUrl ? match[1] : this.replace(match[1], {
